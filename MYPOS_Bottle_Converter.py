...
def convertToDDAExcel(SourceData,DDATemplate):
    ...
    for z in range(len(SourceData)):
        ...
        if pd.notna(SourceData.iloc[z]["stock_id"]):
            TemplateData=DDATemplate.iloc[0]
            TemplateData["ProductCode(15)"] = SourceData.iloc[z]["Number"]
            TemplateData["Description1(100)"] = SourceData.iloc[z]["description"]
            TemplateData["Description2(100)"] = SourceData.iloc[z]["Longdescrip"]
...
def processProductWithBarCode(connect_string):
    ...
    #dpn is department name
    productListQuery = "SELECT plu.*, dept.dpn FROM plu, dept  where plu.dept=dept.Number"
    ...
    productList.to_excel(r'export_dataframe.xlsx', index = True, header=True,engine='xlsxwriter')
    ...
    DDAExcel = DDADataTemplete .head(0)
    print(DDAExcel)

    #DDAExcel = convertToDDAExcel(productList,DDAExcel)
...
class App:
    def __init__(self, root):
        ...
        DB_Name_Box.insert(0,'my_POS_DB')
...
#------------- POS Drop list---------------------------------------------
        ...
        DB_Name_Label["fg"] = "#333333"
        DB_Name_Label["justify"] = "left"
        DB_Name_Label["text"] = "POS Name"
        DB_Name_Label.place(x=50,y=320,width=90,height=25)

        # Create the list of options
        options_list = ["Suntech", "AUPOS", "MYPOS_Bottle"]
        value_inside = tk.StringVar(root)
        value_inside.set("Select an POS")

        # Create the optionmenu widget and passing 
        # the options_list and value_inside to it.
        #POS_OMlist = tk.OptionMenu(root, value_inside, *options_list)
        
        POS_OMlist = ttk.Combobox(root, value_inside, *options_list)
        #POS_OMlist=tk.OptionMenu(root)
        ft = tkFont.Font(family='Times',size=10)
        POS_OMlist["font"] = ft
        POS_OMlist["fg"] = "#333333"
        POS_OMlist["justify"] = "center"
        POS_OMlist["text"] = "RadioButton"
        POS_OMlist.place(x=190,y=320,width=275,height=30)
